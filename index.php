<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <!--[if IE]>
        <meta http-equiv='X-UA-Compatible' content='IE=edge,chrome=1'>
        <![endif]-->
    <meta name="keywords" content="HTML5 Template" />
    <meta name="description"
        content="Fotografo de eventos | Buenos Aires | JFL Fotografia y Filmacion"/>
    <meta name="author" content="itgeeksin.com" />
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags -->
    <title>Fotografo de eventos | Buenos Aires | JFL Fotografia y Filmacion</title>
    <!-- Bootstrap -->
    <!-- Favicon -->
    <link rel="shortcut icon" href="assets/img/favicon.ico" type="image/x-icon">
    <link rel="icon" href="assets/img/favicon.ico" type="image/x-icon">
    <!-- Master Css -->
    <link href="main.css" rel="stylesheet">
    <!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
        <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
        <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
        <![endif]-->

</head>

<body>
    <!--//================preloader  start==============//-->
    <div class="preloader">
        <div class="itg-loader">
            <figure>
                <img src="assets/img/loader/loder.png" alt="" />
            </figure>
            <h4>Cargando</h4>
        </div>
    </div>
    <!--//================preloader  end==============//-->
    <!--//================Header start==============//-->
    <header id="header">
        <div id="main-menu" class="wa-main-menu">
            <!-- Menu -->
            <div class="wathemes-menu relative">
                <!-- navbar -->
                <div class="navbar navbar-default mar0" role="navigation">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-md-12 head-box">
                                <div class="navbar-header">
                                    <!-- Button For Responsive toggle -->
                                    <button type="button" class="navbar-toggle" data-toggle="collapse"
                                        data-target=".navbar-collapse">
                                        <span class="sr-only">Toggle navigation</span>
                                        <span class="icon-bar"></span>
                                        <span class="icon-bar"></span>
                                        <span class="icon-bar"></span>
                                    </button>
                                    <!-- Logo -->
                                    <a class="navbar-brand" href="index.php">
                                        <h3 class="site_logo" style="color: white;">JFL Producciones</h3>
                                    </a>
                                </div>
                                <!-- Navbar Collapse -->
                                <div class="navbar-collapse collapse">
                                    <!-- nav -->
                                    <ul class="nav navbar-nav">
                                        <li><a href="index.php">Inicio</a></li>
                                        <li><a href="about-us.php">Acerca de</a></li>
                                        <li><a href="presentation.php">Presentación</a></li>
                                        <li>
                                            <a href="#">Galeria <i class="fa fa-angle-down hidden-lg hidden-md hidden-sm" aria-hidden="true"></i></a>
                                            <ul class="dropdown-menu">
                                                <li><a href="weddings.php">Bodas</a></li>
                                                <li><a href="#">Cumpleaños de 15</a></li>
                                                <li><a href="#">Eventos</a></li>
                                            </ul>
                                        </li>
                                        <li><a href="contact.php">Contacto</a></li>
                                    </ul>
                                </div>
                                <!-- navbar-collapse -->
                            </div>
                            <!-- col-md-12 -->
                        </div>
                        <!-- row -->
                    </div>
                    <!-- container -->
                </div>
                <!-- navbar -->
            </div>
            <!--  Menu -->
        </div>
    </header>
    <!--//================Header end==============//-->
    <!--//================Slider start==============//-->
    <section id="slider-section">
        <div id="main-slider" class="owl-carousel owl-theme slider">
            <div class="item">
                <figure class="slider-image">
                    <img src="assets/img/slider/slide-1.jpg" alt="" />
                </figure>
                <div class="slider-text text-center">
                    <div class="container">
                        <div class="row">
                            <div class="col-xs-12">
                                <h1>Soluciones fotográficas </h1>
                                <h1>de todo tipo</h1>
                                <p>Imágenes limpias, elegantes e innovadoras, con un acabado profesional sea cual sea el
                                    proyecto.
                                </p>
                                <a href="contact.php" class="itg-button active">Contacto</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="item">
                <figure class="slider-image">
                    <img src="assets/img/slider/slide-2.jpg" alt="" />
                </figure>
                <div class="slider-text text-center">
                    <div class="container">
                        <div class="row">
                            <div class="col-xs-12">
                                <h1>Recursos</h1>
                                <p>Somos fotógrafos profesionales que se ajustan a las necesidades de cada sesión y
                                    sector.
                                </p>
                                <a href="contact.php" class="itg-button active">Contacto</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="item">
                <figure class="slider-image">
                    <img src="assets/img/slider/slide-3.jpg" alt="" />
                </figure>
                <div class="slider-text text-center">
                    <div class="container">
                        <div class="row">
                            <div class="col-xs-12">
                                <h1>Estética</h1>
                                <p>Proporcionamos un acabado visual único que posee la
                                    elegancia característica de JFL Producciones y que ofrece un atractivo artístico
                                    impecable.
                                </p>
                                <a href="contact.php" class="itg-button active">Contacto</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--//================Slider end==============//-->
    <div class="clear  padT100"></div>
    <!--//================Services start==============//-->
    <section>
        <div class="container">
            <div class="row">
                <!-- Theme Heading -->
                <div class="theme-heading">
                    <h1>Nuestros servicios</h1>
                    <h3><span class="heading-shape">Nuestros <strong>servicios</strong></span></h3>
                    <p>Ofrecemos total cobertura de eventos de cualquier envergadura, como pueden
                        ser cumpleaños, casamientos, eventos sociales y empresariales, bautismos, comuniones y muchos
                        más.
                    </p>
                </div>
                <!-- Theme Heading -->
            </div>
        </div>
        <div class="service-background padT100">
            <div class="container">
                <div class="row">
                    <div class="services theme-effet">
                        <div class="col-md-4 col-sm-4 col-xs-12 service-single">
                            <div class="sevice-box theme-hover">
                                <figure>
                                    <img src="assets/img/services/casamiento.jpg" alt="" />
                                    <figcaption>
                                        <div class="content">
                                            <div class="content-box">
                                                <span>CASAMIENTOS</span>
                                                <span></span>
                                                <a href="contact.php" class="itg-button">Solicitar presupuesto</a>
                                            </div>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-12 service-single">
                            <div class="sevice-box theme-hover">
                                <figure>
                                    <img src="assets/img/services/eventos.jpg" alt="" />
                                    <figcaption>
                                        <div class="content">
                                            <div class="content-box">
                                                <span>EVENTOS SOCIALES</span>
                                                <a href="contact.php" class="itg-button">Solicitar presupuesto</a>
                                            </div>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-12 service-single">
                            <div class="sevice-box theme-hover">
                                <figure>
                                    <img src="assets/img/services/cumple15.jpg" alt="" />
                                    <figcaption>
                                        <div class="content">
                                            <div class="content-box">
                                                <span>FIESTA DE 15</span>
                                                <a href="contact.php" class="itg-button">Solicitar presupuesto</a>
                                            </div>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
<!-------------------------- Segundo servicios ---------------------------------->
                        <div class="clear padT100 "></div>
                        <div class="col-md-4 col-sm-4 col-xs-12 service-single">
                            <div class="sevice-box theme-hover">
                                <figure>
                                    <img src="assets/img/services/bautismo.jpg" alt="" />
                                    <figcaption>
                                        <div class="content">
                                            <div class="content-box">
                                                <span>BAUTISMOS</span>
                                                <span></span>
                                                <a href="contact.php" class="itg-button">Solicitar presupuesto</a>
                                            </div>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-12 service-single">
                            <div class="sevice-box theme-hover">
                                <figure>
                                    <img src="assets/img/services/comunion.jpg" alt="" />
                                    <figcaption>
                                        <div class="content">
                                            <div class="content-box">
                                                <span>COMUNIÓN</span>
                                                <a href="contact.php" class="itg-button">Solicitar presupuesto</a>
                                            </div>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                        <div class="col-md-4 col-sm-4 col-xs-12 service-single">
                            <div class="sevice-box theme-hover">
                                <figure>
                                    <img src="assets/img/services/evento.jpg" alt="" />
                                    <figcaption>
                                        <div class="content">
                                            <div class="content-box">
                                                <span>EVENTOS EMPRESARIALES</span>
                                                <a href="contact.php" class="itg-button">Solicitar presupuesto</a>
                                            </div>
                                        </div>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--//================Services end==============//-->
    <div class="clear padB50"></div>
    <!--//================Contact start==============//-->
    <section>
        <div class="contact-us padT100 padB100">
            <div class="container">
                <div class="row">
                    <div class="col-md-6 col-sm-6 col-xs-12" style="padding-left: 10%;">
                        <div class="row">
                            <div class="address">
                                <h3>Contacto</h3>
                                <ul>
                                    <li><span>Dirección:</span>12 civil line Dewas</li>
                                    <li><span>Teléfono:</span>(0727272) 403533 </li>
                                    <li><span>Email:</span>info@itgeeksin.com</li>
                                    <li><span>Website:</span>www.itgeeksin.com</li>
                                    <li><span>Horarios de atención:</span>Lunes a viernes de 8 AM a 19 PM</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-6 col-xs-12" style="padding-left: 15%;">
                        <div class="social">
                            <h3>Seguinos en :</h3>
                            <ul class="social-icon">
                                <li><a href="#" class="active"><span><i class="fa fa-twitter"
                                                aria-hidden="true"></i></span></a></li>
                                <li><a href="#"><span><i class="fa fa-facebook" aria-hidden="true"></i></span></a></li>
                                <li><a href="#"><span><i class="fa fa-dribbble" aria-hidden="true"></i></span></a></li>
                                <li><a href="#"><span><i class="fa fa-instagram" aria-hidden="true"></i></span></a></li>
                                <li><a href="#"><span><i class="fa fa-pinterest" aria-hidden="true"></i></span></a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--//================Contact end==============//-->
    <div class="clear"></div>
    <!--//================Footer start==============//-->
    <footer class="main_footer">
        <div class="bottom-footer padTB20">
            <div class="container">
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        <p style="text-align: center;">Copyright 2020 All Right Reserved <a
                                href="#"><span class="theme-color">DevCode</span></a></p>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <!-- scroll top button -->
    <a id="scroll-top"><i class="fa fa-angle-double-up" aria-hidden="true"></i></a>
    <!--//================Footer end==============//-->
    <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
    <script src="assets/js/jquery.min.js"></script>
    <script src="assets/js/bootstrap.min.js"></script>
    <script src="assets/plugin/megamenu/js/hover-dropdown-menu.js"></script>
    <script src="assets/plugin/megamenu/js/jquery.hover-dropdown-menu-addon.js"></script>
    <script src="assets/plugin/owl-carousel/js/owl.carousel.min.js"></script>
    <script src="assets/plugin/fancyBox/js/jquery.fancybox.pack.js"></script>
    <script src="assets/plugin/fancyBox/js/jquery.fancybox-media.js"></script>
    <script src="assets/plugin/mixItUp/js/jquery.mixitup.js"></script>
    <script
        src="https://maps.googleapis.com/maps/api/js?v=3.exp&amp;key=AIzaSyCIdIjmZwQXmxM_eNnEPMp1YQm4DFXpkkc"></script>
    <script src="assets/js/main.js"></script>
</body>

<!-- Mirrored from preview.itgeeksin.com/photography/ by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 31 Dec 2019 01:56:02 GMT -->

</html>